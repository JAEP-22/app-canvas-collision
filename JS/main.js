...
class Circle {
    constructor(x, y, radius, color, speed) {
        ...
        this.color = color;
        this.speed = speed;
        this.dx = Math.sin(x); // Inicializa el movimiento en S
        this.dy = -Math.abs(speed); // Movimiento hacia arriba
        this.opacity = 1;
        this.fading = false;
        this.angle = 0; // Ángulo inicial para el movimiento en S
    }

    draw(context) {
        const gradient = context.createRadialGradient(this.posX, this.posY, this.radius * 0.3, this.posX, this.posY, this.radius);
        gradient.addColorStop(0, this.color);
        gradient.addColorStop(1, "#000");

        context.beginPath();
        context.fillStyle = gradient;
        context.strokeStyle = "#fff"; // White border for better visibility
        context.lineWidth = 2;
    ...
    update(context) {
        ...
        this.angle += 0.05; // Incremento del ángulo para el movimiento en S
        this.posX += Math.sin(this.angle) * this.speed; // Movimiento en S en el eje X
    ...
    changeColorIfMouseOver(mouseX, mouseY) {
        ...
        if (distance < this.radius) {
            this.color = "#FF0000"; // Change to red
        } else {
